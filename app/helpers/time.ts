...
const getUpcomingMidnight = (): Date => {
...
const getNextThursdayMidnightUTC = (): Date => {
  const today = new Date();
  const dayOfWeek: number = today.getUTCDay();
  const daysUntilThursday: number =
    dayOfWeek <= 4 ? 4 - dayOfWeek : 11 - dayOfWeek;
  const nextThursday: Date = new Date(
    today.getUTCFullYear(),
    today.getUTCMonth(),
    today.getUTCDate() + daysUntilThursday
  );
  nextThursday.setUTCHours(0, 0, 0, 0);
  return nextThursday;
};

const formatTime = (remainingTime: number, type: CountdownType): string => {
...
const getNewUTCDate = (now = new Date()) => {
...
const getPreviousMidnightUTC = (): Date => {
  const today = getNewUTCDate();
  const yesterday = getNewUTCDate(today);
  yesterday.setUTCDate(today.getUTCDate()); // Set the date to yesterday
  yesterday.setUTCHours(0, 0, 0, 0); // Set time to midnight UTC
  return yesterday;
};

const getNewEndTime = (type: CountdownType) => {
  switch (type) {
    case "weekly":
      return getNextThursdayMidnightUTC();
    default:
      return getUpcomingMidnight();
  }
};

const getNextEventInfo = (): NextEvent => {
  const now = getNewUTCDate();
  ...
  for (const [time, name] of Object.entries(events)) {
    ...
    if (currTimeToNumber > nowToMilliseconds && currTimeToNumber < eventTime) {
